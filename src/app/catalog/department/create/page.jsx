...
export default function CreateDepartmentPage() {
  ...
  const fetchCategories = async () => {
    ...
    try {
      const res = await fetch('/api/catalog/categories?pageSize=200');
      const json = await res.json();
      if (json.success) setCategories(json.data.records);
    } catch (err) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (json.success) {
        ...
        showToast(json.message || 'Failed to create department', 'error');
  ...
  return (
    <div className="font-sans text-sm">
      ...
      <div className="flex items-start justify-between mb-4">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Create Department</h1>
