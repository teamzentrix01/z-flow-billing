...
const initialForm = { name: '', code: '', is_active: true };

export default function CreateIncomeHeadPage() {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch('/api/catalog/income-heads', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) });
      const json = await res.json();
      if (json.success) {
        showToast('Income Head created');
        setTimeout(() => router.push('/catalog/income-head'), 800);
      } else {
    ...
    <div className="min-h-screen bg-gray-100 font-sans text-sm">
      ...
      <div className="px-6 pt-5 pb-1">
        <nav className="flex items-center gap-1.5 text-xs text-gray-500">
          ...
          <span>›</span>
          <span className="text-gray-700 font-medium">Create Income Head</span>
        </nav>
      </div>

      <div className="flex items-start justify-between px-6 py-4">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Create Income Head</h1>
          <p className="text-xs text-gray-500 mt-0.5">Create Income Head description</p>
        </div>
