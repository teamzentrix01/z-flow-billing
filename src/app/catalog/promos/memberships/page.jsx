...
export default function MembershipsListPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/catalog/memberships?${params.toString()}`);
      const json = await res.json();
      if (!json.success) throw new Error(json.message || 'Failed to load memberships');
  ...
  const rows = useMemo(
    () =>
      records.map((record, index) => ({
        ...
        membershipPrice: `₹${Number(record.price || 0).toFixed(2)}`,
        validity: record.validity_label || record.validity || '—',
        category: record.category_name || '—',
        subCategory: record.sub_category_name || '—',
        products: record.product_count ?? 0,
        storePrice: record.store_price_label || `₹${Number(record.price || 0).toFixed(2)}`,
      })),
  ...
  return (
    <PageShell>
      <nav className="mb-4 flex items-center gap-1.5 text-xs text-gray-500">
        ...
        <span>›</span>
        <span className="font-medium text-gray-700">Memberships</span>
      </nav>

      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-[22px] font-semibold text-gray-900">Membership</h1>
        ...
        <Link
          href="/catalog/promos/memberships/create"
          ...
          <table className="w-full min-w-[960px] border-collapse text-sm">
            <thead className="bg-slate-50 text-slate-800">
              <tr>
                ...
                <th className="px-3 py-3 text-left font-semibold">Membership ⇅</th>
                <th className="px-3 py-3 text-left font-semibold">Price ⇅</th>
                <th className="px-3 py-3 text-left font-semibold">Validity ⇅</th>
                <th className="px-3 py-3 text-left font-semibold">Category ⇅</th>
                <th className="px-3 py-3 text-left font-semibold">Sub Category ⇅</th>
                <th className="px-3 py-3 text-left font-semibold">Products ⇅</th>
                <th className="px-3 py-3 text-left font-semibold">Store Price ⇅</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                rows.map((row) => (
                  <tr key={row.id} className="border-t border-gray-100 hover:bg-slate-50/60">
                    ...
                    <td className="px-3 py-3 text-gray-800">{row.membershipPrice}</td>
                    <td className="px-3 py-3 text-gray-700">{row.validity}</td>
                    <td className="px-3 py-3 text-gray-700">{row.category}</td>
                    <td className="px-3 py-3 text-gray-700">{row.subCategory}</td>
                    <td className="px-3 py-3 text-gray-700">{row.products}</td>
                    <td className="px-3 py-3 text-gray-700">{row.storePrice}</td>
                  </tr>
                ))
              ) : (
                <tr>
